...
class FindResultView(Gtk.HBox):
	def __init__(self, window, result_gui_settings, plugin):
		...
		self.findResultTreeview.set_grid_lines(int(self.result_format['GRID_PATTERN']))	
		...
		if self.result_format['SHOW_HEADERS']:
			...
			for i in range(0, column_num):
				self.findResultTreeview.get_column(i).set_sizing(1)
		...
		self.settingsButton = Gtk.Button(_("Find/Replace"))
		self.settingsButton.set_no_show_all(True)
		self.settingsButton.connect("clicked", self.on_settings_clicked_action)
		...
		v_buttonbox.pack_start(self.settingsButton, False, False, 5)
		...
		self.settingsItem = Gtk.MenuItem.new_with_label(_('Find/Replace'))
		...
		self.contextMenu.append(self.settingsItem)
		...
		self.settingsItem.connect('activate', self.on_settings_activate)
		...
		self.showSettingsButtonItem = Gtk.CheckMenuItem.new_with_label(_('Find/Replace'))
		...
		self.showButtonsSubmenu.append(self.showSettingsButtonItem)
		...
		self.showSettingsButtonItem.connect('activate', self.on_showSettingsButtonItem_activate)
		...
		self.showSettingsButtonItem.show()
	...
	def on_settings_activate(self, object):
		self._plugin.advanced_find_active(self._plugin,12)
	...
	def on_showSettingsButtonItem_activate(self, object):
		if self.showSettingsButtonItem.get_active() == True:
			self.result_gui_settings['SETTINGS_BUTTON'] = True
			self.settingsButton.show()
		else:
			self.result_gui_settings['SETTINGS_BUTTON'] = False
	...
	def on_settings_clicked_action(self, object):
		self._plugin.advanced_find_active(self._plugin,12)
	...
	def show_buttons(self):
		...
		if self.result_gui_settings['SETTINGS_BUTTON'] == True:
			self.selectNextButton.show()
			self.showSettingsButtonItem.set_active(True)
